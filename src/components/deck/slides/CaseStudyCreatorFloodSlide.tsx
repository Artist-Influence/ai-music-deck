...
const richBrian = {
  ...
  overview: 'Large-scale creator activation to rapidly increase audio usage and establish algorithmic momentum.',
  metrics: [
...
const majorLazer = {
  ...
  artist: 'Major Lazer',
  track: 'MINI SKIRT',
  overview: 'Pushed official audio into trending environments across Shorts, Reels, and Facebook for rapid visibility.',
  metrics: [
...
const CaseStudyCreatorFloodSlide = () => (
  <div className="w-full h-full bg-background relative overflow-hidden px-5 pt-5 pb-3 md:px-8 md:pt-6 md:pb-4 flex flex-col items-center">
    <div className="relative z-10 flex flex-col h-full w-full max-w-[1400px]">
      <h1 className="text-xl md:text-4xl font-bold text-foreground mb-1 md:mb-2">Scale on impact.</h1>
      <p className="text-sm md:text-lg text-muted-foreground mb-3 md:mb-4">Creator Flood &amp; Trending Campaigns</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-[1400px] mx-auto flex-1 min-h-0">
        ...
        <GlassPanel variant="bright" className="p-3 md:p-4 flex flex-col h-full">
          <p className="text-xs md:text-base text-primary font-mono mb-1 md:mb-2 tracking-widest">{richBrian.tag}</p>
          <p className="text-lg md:text-3xl font-bold text-foreground">{richBrian.artist}</p>
          <p className="text-xs md:text-sm text-muted-foreground mb-2 md:mb-3">{richBrian.track}</p>
          <p className="text-xs md:text-base text-muted-foreground mb-2 md:mb-3">{richBrian.overview}</p>
        ...
        {/* Major Lazer */}
        <GlassPanel variant="bright" className="p-3 md:p-4 flex flex-col h-full">
          <p className="text-xs md:text-base text-primary font-mono mb-1 md:mb-2 tracking-widest">{majorLazer.tag}</p>
          <p className="text-lg md:text-3xl font-bold text-foreground">{majorLazer.artist}</p>
          <p className="text-xs md:text-sm text-muted-foreground mb-2 md:mb-3">{majorLazer.track}</p>
          <p className="text-xs md:text-base text-muted-foreground mb-2 md:mb-3">{majorLazer.overview}</p>
